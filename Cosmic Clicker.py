...
def spawn_star():
    ...
    stars.append({
        ...
        "drift_amplitude": random.uniform(10, 30),
        "drift_speed": random.uniform(0.02, 0.08),
...
def move_stars():
    ...
    for star in stars[:]:
        star["angle"] += star["drift_speed"]
        dx = star["speed_x"] + math.sin(star["angle"]) * star["drift_amplitude"] * star["drift_speed"]
        dy = star["speed_y"]

        canvas.move(star["id"], dx, dy)
        x1, y1, x2, y2 = canvas.coords(star["id"])
